...
public class ChessBoard implements Serializable{
	static final int NUM_OF_ROW = 8;
	static final int NUM_OF_COLUMN = 8;
    /**
     * Default constructor
     */
   ...
   protected HashSet<Coord> coorPieceMovable;
     ...
     */
    public boolean simulation(Coord startC, Coord finalC) { //gia lap quan co o vi tri startC se di den finalC thi se ntn
    	...
    	if(this.game.getTurn()) {
    		for(Coord c: this.game.blackPlayer.getCoordOfMyPieces()) {
    			//cAttacked.addAll(this.board[c.getR()][c.getC()].possibleMove(c));  //Origin
    			if (tmp!=null && (c.row == finalC.row && c.column == finalC.column)) { 
    				//Neu finalC co quan va se an thi ko can ktra quan o day co chieu vua hien thoi ko.
    			} else {
    				//System.out.println("c.getR(): " + c.getR() + " ; c.getC(): " + c.getC());
    				cAttacked.addAll(this.board[c.getR()][c.getC()].possibleMove(c));
    			}
    		}
    		//Neu quan di chuyen ko phai la vua thi kiem tra neu di chuyen thi co chieu vao vua hien tai ko
    		if( this.board[finalC.getR()][finalC.getC()].toString()!= "K"   && cAttacked.contains(this.getWhiteKingCoord())) {
    			accepted=false;
    		}
    		//Neu quan di chuyen la vua thi khi di chuyen den cho moi thi ktra quan dich co chieu khi o vi tri moi ko.
    		if( this.board[finalC.getR()][finalC.getC()].toString()== "K"  && cAttacked.contains(finalC)) {
    			accepted=false;
    		}
    	}else {
    		for(Coord c: this.game.whitePlayer.getCoordOfMyPieces()) {
    			//cAttacked.addAll(this.board[c.getR()][c.getC()].possibleMove(c));
    			//*** test v4
    			if (tmp!=null && (c.row == finalC.row && c.column == finalC.column)) { //Neu finalC co quan va se an thi ko can ktra quan o day co chieu vua hien thoi ko.
    		...
    		*/
    		//Neu quan di chuyen ko phai la vua thi kiem tra neu di chuyen thi co chieu vao vua hien tai ko
    		if( this.board[finalC.getR()][finalC.getC()].toString()!= "k"   && cAttacked.contains(this.getBlackKingCoord())) {
    			accepted=false;
    		}
    		//Neu quan di chuyen la vua thi khi di chuyen den cho moi thi ktra quan dich co chieu khi o vi tri moi ko.
    		if( this.board[finalC.getR()][finalC.getC()].toString()== "k"  && cAttacked.contains(finalC)) {

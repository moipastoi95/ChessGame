...
public class Game {
	...
	public int courseOfTheGame() {
		...
		do {
			...
			b = false;
			while (b == false) { // demander si l'utilisateur veut jouer cette pi�ce ou selecitonner une autre
									// pi�ce (boucle while)
				System.out.println("\nCoorPieceMovable:" + this.cb.getCoorPieceMovable());
				System.out.println("What piece do you want play?");
				cStart = Input.askValidCoord();

				try { // elle fait avec une exeception try catch pour voir si la coordonn�
						// selectionn� correspond bien aune coord de coorPieceMovable(boucle while si
						// non pour redemander), si ok afficher les possibilit�s
					select(cStart);
			...
			// si le joueur veut joeur cette pi�ce demande coordonn�e d'arriv� de la
			// pi�ce selectionn�
			b = false;
			while (b == false) {
				...
				try {
					...
					// pour oir si cette coordonn�e fait partie des LegalMove de la pi�ce de la
					// case de d�part(boucle while si non pour redemander)

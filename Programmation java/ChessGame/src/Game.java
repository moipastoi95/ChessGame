	...
	public int courseOfTheGame() {
		...
		while (!(this.cb.getCoorPieceMovable().isEmpty())) {
			...
			while (b == false) { // demander si l'utilisateur veut jouer cette pièce ou selecitonner une autre
				...
				cStart = Input.askValidCoord(); //Dung de lay ve toa do cua quan can di chuyen
				...
				try { // elle fait avec une exeception try catch pour voir si la coordonné
					...
					HashSet<Coord> selection = select(cStart); //Kiem tra o go vao co trong coorPieceMovable ko, neu co thi goi ham getAllowedMove() 
					System.out.println(selection);				//de tra ve mang allowMove cua quan co do. 
					b = Input.askValidYesNo();	//Hoi lai xem co dung la muon di chuyen o nay ko?
			...
			while (b == false) {
				...
				cFinal = Input.askValidCoord(); //Dung de lay ve toa do can di chuyen den
				...
				try {
					...
					play(cStart, cFinal); //Kiem tra xem nhung o co the di (cua quan dang o cStart) co chua cFinal ko?
					b = true;

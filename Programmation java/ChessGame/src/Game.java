   ...
   public void play(Coord cStart, Coord cFinal) throws NotInHashSetException{
    ...
    public int courseOfTheGame() {
    	...
    	do {
    		...
    		b=false;
    		while(b==false ) { //demander si l'utilisateur veut jouer cette pi�ce ou selecitonner une autre pi�ce (boucle while)
    			System.out.println("\nCoorPieceMovable:"+this.cb.getCoorPieceMovable());
    			System.out.println("What piece do you want play?");
        		cStart=Input.askValidCoord();

        		try{ //elle fait avec une  exeception try catch pour voir si la coordonn� selectionn� correspond bien aune coord de coorPieceMovable(boucle while si non pour redemander), si ok afficher les possibilit�s
        			select(cStart);
